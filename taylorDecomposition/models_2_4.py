    ...
    def __call__(self, X, reuse=False):
        with tf.variable_scope(self.name) as scope:
            ...
            with tf.variable_scope('layer2'):
                conv2 = tf.layers.conv2d(inputs=pool1, filters=256, kernel_size=[3, 3], padding="SAME",
                                         activation=tf.nn.relu, use_bias=False)
                pool2 = tf.layers.max_pooling2d(inputs=conv2, pool_size=[2, 2], padding="SAME", strides=2)

            # Convolutional Layer #3 and Pooling Layer #3
            with tf.variable_scope('layer3'):
                conv3 = tf.layers.conv2d(inputs=pool2, filters=512, kernel_size=[3, 3], padding="SAME",
                                         activation=tf.nn.relu, use_bias=False)
            ...
            with tf.variable_scope('layer4'):
                ...
                dense4 = tf.layers.dense(inputs=flat, units=1024, activation=tf.nn.relu, use_bias=False)

            # Logits (no activation) Layer: L5 Final FC 625 inputs -> 10 outputs
            with tf.variable_scope('layer5'):
                logits = tf.layers.dense(inputs=dense4, units=2, use_bias=False)
