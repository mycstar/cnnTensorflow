    ...
    def __call__(self, X, reuse=False):
        with tf.variable_scope(self.name) as scope:
            ...
            with tf.variable_scope('layer0'):
                X_img = tf.reshape(X, [-1, 1, 21000, 1], name="l0_reshape")

            # Convolutional Layer #1 and Pooling Layer #1
            with tf.variable_scope('layer1'):
                conv1 = tf.layers.conv2d(inputs=X_img, filters=128, kernel_size=[3, 3], padding="SAME",
                                         activation=tf.nn.relu, use_bias=False, name="l1c1_conv2d")
                pool1 = tf.layers.max_pooling2d(inputs=conv1, pool_size=[2, 2], padding="SAME", strides=2,
                                                name="l1p1_max_pooling2d")

            # Convolutional Layer #2 and Pooling Layer #2
            with tf.variable_scope('layer2'):
                conv2 = tf.layers.conv2d(inputs=pool1, filters=128, kernel_size=[3, 3], padding="SAME",
                                         activation=tf.nn.relu, use_bias=False, name="l2c2_conv2d")
                pool2 = tf.layers.max_pooling2d(inputs=conv2, pool_size=[2, 2], padding="SAME", strides=2,
                                                name="l2p2_max_pooling2d")

            # Convolutional Layer #3 and Pooling Layer #3
            with tf.variable_scope('layer3'):
                conv3 = tf.layers.conv2d(inputs=pool2, filters=128, kernel_size=[3, 3], padding="SAME",
                                         activation=tf.nn.relu, use_bias=False,name="l3c3_conv2d")
                pool3 = tf.layers.max_pooling2d(inputs=conv3, pool_size=[2, 2], padding="SAME", strides=2,name="l3p3_max_pooling2d")

            # Dense Layer with Relu
            with tf.variable_scope('layer4'):
                flat = tf.keras.layers.Flatten(name="l4c4_Flatten")(pool3)
                dense4 = tf.layers.dense(inputs=flat, units=256, activation=tf.nn.relu, use_bias=False,
                                         name="l4p4_dense")

            # Logits (no activation) Layer: L5 Final FC 625 inputs -> 10 outputs
            with tf.variable_scope('layer5'):
                logits = tf.layers.dense(inputs=dense4, units=1074, use_bias=False, name="l5logits_dense")
                prediction = tf.nn.softmax(logits, name="l5prediction_softmax")
            ...
            for i in range(len(self.activations) - 1):
                ...
                if i is self.last_ind:
                    ...
                    if 'conv' in self.activations[i].name.lower():
                        Rs.append(self.backprop_conv_input(self.activations[i + 1], self.weights[j], Rs[-1],
                                                           self.conv_strides))
                    else:
                ...
                if i is 0:
                    ...
                    # Rs.append(
                    #     self.backprop_pool(self.activations[i + 1], Rs[-1], self.pool_ksize, self.pool_strides, 'avg'))
                else:
                    raise NameError('Unknown operation.')
