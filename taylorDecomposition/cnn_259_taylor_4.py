...
y_pred = tf.keras.layers.Conv2D(filters=256, kernel_size=[3, 3], strides=(1, 1), padding='same',
                                 activation='relu')(y_pred)
y_pred = tf.keras.layers.MaxPool2D(pool_size=(1, 1), strides=(1, 1))(y_pred)

y_pred = tf.keras.layers.Conv2D(filters=512, kernel_size=[3, 3], strides=(1, 1), padding='same',
                                 activation='relu')(y_pred)
y_pred = tf.keras.layers.MaxPool2D(pool_size=(1, 1), strides=(1, 1))(y_pred)

y_pred = tf.keras.layers.Conv2D(filters=768, kernel_size=[3, 3], strides=(1, 1), padding='same',
                                 activation='relu')(y_pred)
y_pred = tf.keras.layers.MaxPool2D(pool_size=(1, 1), strides=(1, 1))(y_pred)

y_pred = tf.keras.layers.Flatten()(y_pred)
y_pred = tf.keras.layers.Dense(1024, activation='relu')(y_pred)

y_pred = tf.keras.layers.Dense(num_classes, activation='softmax', name="absolute_output")(y_pred)
...
hmaps = run_sensitivity_analysis(checkpoints, num_classes, num_features, collection_name)
print("----relevance scores: %s" % hmaps)

nhmaps = numpy.array(hmaps)
numpy.save("hmaps", nhmaps)
nhmaps2 = numpy.load("hmaps.npy")
print("relevance scores: %s" % nhmaps2)
